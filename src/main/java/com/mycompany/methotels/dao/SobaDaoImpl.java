...
public class SobaDaoImpl implements SobaDao{
    ...
    public Soba getSobaById(Integer sobaid) {
        return (Soba) session.createCriteria(Soba.class).add(Restrictions.eq("sobaid",sobaid)).uniqueResult();
    }
    ...
    public void obrisiSobu(int sobaid) {
        Soba soba = (Soba) session.createCriteria(Soba.class).add(Restrictions.eq("sobaid", sobaid)).uniqueResult();
        session.delete(soba);
    }
